...
const Services = forwardRef((props, ref) => {
    ...
    return (
        <div ref={ref} className='flex flex-col  bg-yellow-950 items-center py-4  '>
            <h1 className='lg:text-4xl text-2xl  text-yellow-50 font-kanit font-bold '>Our Services</h1>
...

                {
                    services.map((item, index) => (
                        <div key={index} className='flex flex-col   items-center'>
                            <div className='lg:h-52 lg:w-52 h-32 w-32  bg-neutral-950 relative  outline-yellow-900  rounded-2xl group  '>
                                ...
                                <div className='flex absolute opacity-0  inset-0 justify-center  items-center group-hover:opacity-100 transition-opacity duration-300'>
                                    <button onClick={handleClick} className=' text-sm bg-yellow-400 font-kanit text-yellow-900 outline-none outline-yellow-900 py-2 px-4   rounded-xl lg:text-lg '>Play Now </button>
                                </div>
                            </div>
                            <h1 className=' text-yellow-50 lg:text-lg text-sm  mt-2 font-kanit'>{item.name}</h1>
                        </div>
                    ))
                }
            </div>
